...
```mermaid
flowchart TD
    Start([用户提交消息]) --> Validate[校验 backend / npc / message / rerank 配置]
    Validate --> GetMem[获取 Memory 实例]
    GetMem --> RecallMV{recall_memory_vector?}
    RecallMV -->|否| NoVec[long_results 为空]
    RecallMV -->|是| VecBranch{MongoDB + pre-filter?}
    VecBranch -->|否: post-filter 或 Postgres| Mem0Search[mem0 memory.search]
    ...
    Classify --> WikiGate{recall_wiki 且 MongoDB 且 GAME_WIKI 且 URI?}
...
sequenceDiagram
    ...
    App->>DB: ensure_postgres_resources
    alt POSTGRES_URI 已配置
        DB->>P: CREATE EXTENSION vector
    end
    Note over DB: 任一环节失败则启动失败 fail-fast
```
